import { IPermisConfig, MutableConfig, resolveConfig } from "./config";
import {
  UsersApi,
  TenantsApi,
  RolesApi,
...
export class Permis {
   ...
   */
  public readonly api: IPermisApi;
   ...
   */
  constructor(config: IPermisConfig) {
  ...
  async checkWithDetails(request: ICheckRequest): Promise<ICheckResponse> {
    ...
    try {
      ...
      if (this.config.debug) {
        console.log(
          `[Permis Debug] Check: user=${userKey}, action=${request.action}, resource=${request.resource}`
        );
        console.log(
          `[Permis Debug] Required permission: ${requiredPermission}`
        );
      }
      ...
      if (this.config.debug) {
        console.log(`[Permis Debug] User's role keys:`, roleKeys);
      }
      ...
      if (this.config.debug) {
        console.log(
          `[Permis Debug] Available roles:`,
          Array.from(rolesMap.keys())
        );
        for (const [key, role] of rolesMap) {
          console.log(
            `[Permis Debug] Role "${key}" permissions:`,
            role.permissions
          );
        }
      }
      ...
      for (const roleKey of roleKeys) {
        ...
        if (this.config.debug) {
          console.log(
            `[Permis Debug] Role "${roleKey}" has permissions:`,
            permissions
          );
        }
        if (
      ...
      if (this.config.debug) {
        console.log(
          `[Permis Debug] Result: allowed=${allowed}, matchedRoles=`,
          matchedRoles
        );
      }
   ...
   *
   * @throws {PermisApiError} If the user is not allowed
   */
  async checkAndThrow(request: ICheckRequest): Promise<void> {
    const response = await this.checkWithDetails(request);

    if (!response.allowed) {
      throw new PermisApiError({
  ...
  private async fetchAndSetScope(): Promise<void> {
    try {
      ...
      if (this.config.debug) {
        console.debug("[Permis.io SDK] Auto-fetched scope:", {
      ...
      if (!this.config.hasScope()) {
        throw new Error(
          "Permis.io SDK: Failed to fetch API key scope. " +
...
export function createPermis(config: IPermisConfig): Permis {
  return new Permis(config);
}
